...
def create_fresh_note():
    try:
        ...
        payload = {
            "title": "OFFICIAL Discord Welcome",
            "content": NEW_CONTENT,
            "type": "note",
            "created": "2026-02-24T00:05:00.000Z",
            "updated": "2026-02-24T00:05:00.000Z"
        }
        
        # Create fresh
        response = requests.post(f"{DB_URL}?access_token={token}", json=payload)
        
        # Delete old one to avoid confusion
        old_note_url = "https://winslow-756c3-default-rtdb.firebaseio.com/workspaces/winslow_main/notes/-OmBRoxnL6uH2SELpyse.json"
        requests.delete(f"{old_note_url}?access_token={token}")

        if response.status_code == 200:
            print("Fresh note created. Old note deleted.")
        else:
