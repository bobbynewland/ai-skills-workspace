#!/usr/bin/env python3
"""
GLM Swarm Scheduler - Distributes tasks across 10 GLM keys (via Modal)
"""
...
# Load keys
KEYS_FILE = "/root/.openclaw/workspace/.keys/glm.key"

def load_keys():
    """Load all GLM API keys"""
...
def load_usage():
    """Load usage tracking"""
    usage_file = "/root/.openclaw/workspace/.glm_swarm_usage.json"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print("Usage:")
        print("  python3 glm-swarm-scheduler.py status     # Show all keys")
        print("  python3 glm-swarm-scheduler.py get-key    # Get next available key")
        print("  python3 glm-swarm-scheduler.py run <prompt>  # Run task with least-used key")
        print("  python3 glm-swarm-scheduler.py use-key <1-10> <prompt>  # Use specific key")

# Rate limiting - 30 RPM
RPM = 30
REQUEST_DELAY = 60.0 / RPM

_key_last_request_glm = {}

def get_least_used_key_glm():
    ...
    available = []
    for i in range(1, len(keys) + 1):
        last_used = _key_last_request_glm.get(i, 0)
        if current_time - last_used >= REQUEST_DELAY:
            available.append(i)
    
    if not available:
        wait_times = [(i, REQUEST_DELAY - (current_time - _key_last_request_glm.get(i, 0))) 
                      for i in range(1, len(keys) + 1)]
        ...
        available = [i for i in range(1, len(keys) + 1) 
                     if current_time - _key_last_request_glm.get(i, 0) >= REQUEST_DELAY]
    
    available.sort(key=lambda x: usage.get(str(x), {}).get("requests", 0))
    key_index = available[0] - 1
    _key_last_request_glm[available[0]] = current_time
